...
Each series folder contains a `clean.sh` script that lists (and optionally deletes) all generated data files. Files in `data/` can always be regenerated by re-running the notebooks.

```bash
# Dry run — see what would be deleted, no files touched
./perturb_seq/clean.sh
./theis_ecosystem/clean.sh
./spatial_transcriptomics/clean.sh

# Actually delete
./perturb_seq/clean.sh --force           # frees ~5 GB (includes raw Figshare downloads)
./theis_ecosystem/clean.sh --force
./spatial_transcriptomics/clean.sh --force
```
